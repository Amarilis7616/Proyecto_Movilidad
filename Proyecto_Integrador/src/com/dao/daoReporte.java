...
public class daoReporte {
	...
	public List<ReporteInfor>consultaAument() {
		String sql = "select region_p, round(sum(dim_porcentaje_cambio.parks_percent_change_from_baseline)/count(*)) as parques from dim_porcentaje_cambio,fac_report where dim_porcentaje_cambio.sk_porcentaje_cambio=fac_report.sk_porcentaje_cambio and region_p in('DENMARK','NORWAY','NETHERLANDS','GERMANY','FINLAND','LITHUANIA','SOUTH KOREA', 'SWEDEN','SLOVAKIA','BELARUS','HUNGARY','CANADA','CZECHIA','SWITZERLAND', 'UNITED STATES') group by fac_report.region_p ORDER by parques  DESC";
		ResultSet rs = null;
		List<ReporteInfor> datos = new ArrayList<ReporteInfor>();
		Cls_conexion con = new Cls_conexion();
		try {
